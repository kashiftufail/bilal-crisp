module Authentication
  mattr_accessor :login_regex, :bad_login_message, 
    ...
    :email_name_regex, :domain_head_regex, :domain_tld_regex, :email_regex, :bad_email_message, :home_regex, :invalid_home_error
  ...
  self.invalid_home_error = "should be a valid".freeze

  self.email_name_regex  = '[\w\.%\+\-]+'.freeze
  self.domain_head_regex = '(?:[A-Z0-9\-]+\.)+'.freeze
  self.domain_tld_regex  = '(?:[A-Z]{2}|com|org|net|edu|gov|mil|biz|info|mobi|name|aero|jobs|museum)'.freeze
  self.email_regex       = /\A#{email_name_regex}@#{domain_head_regex}#{domain_tld_regex}\z/i
  self.home_regex      =    /^(\s?\d{4}|\(?07\d{3}\)?)\s?\d{3}\s?\d{3}/i
  self.bad_email_message = "is invalid".freeze
  self.invalid_home_error = "is invalid".freeze
